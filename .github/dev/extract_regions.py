...
def generate_terraform_variable(
    ...
    mappings: dict[str, dict],
    output_dir: Path,
    geographic_aliases: frozenset[str] | None = None,
    include_invalid: bool = False,
) -> Path:
    """Generate a variables_generated.tf file with atlas_to_{provider}_region variable."""
    provider_lower = provider.lower()
    var_name = f"atlas_to_{provider_lower}_region"
    ...
    for atlas_region, info in sorted(mappings.items()):
        ...
        if geographic_aliases and atlas_region in geographic_aliases:
            continue
    ...
    lines.append(
        f"    Atlas to {provider_up} region mapping."
        f" Keys = Atlas format, values = {provider_up} format."
    )
    lines.append(
...
def main():
    ...
    if provider:
        ...
        if provider_upper == "GCP":
            output_file = generate_terraform_variable(
                provider,
