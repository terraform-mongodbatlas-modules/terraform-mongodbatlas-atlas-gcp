...
locals {
  endpoint_ip   = local.create ? google_compute_address.atlas[0].address : var.byo.ip_address
  endpoint_name = local.create ? google_compute_forwarding_rule.atlas[0].name : var.byo.forwarding_rule_name
  gcp_project   = local.create ? data.google_compute_subnetwork.atlas[0].project : data.google_client_config.current.project
}
